...
import type { ClientRateLimitConfig, ConfigStatus, FinalFallbackConfig, ImageProxyConfig, ImageUsageSummary, KeyUsageSummary, ModelRateLimitConfig, ModelRewriteConfig, TrafficSummary } from '../shared/types';
import { api } from './api';
...
export function Dashboard({ lang, setLang, onLogout }: { lang: Lang; setLang: (l: Lang) => void; onLogout: () => void }) {
  ...
  async function refresh() {
    try {
      setError('');
      const [c, k, a, rw, ff, rl, crl, ip, iu] = await Promise.all([api<ConfigStatus>('/api/config/status'), api<KeyUsageSummary[]>('/api/keys/usage'), api<Audit[]>('/api/audit'), api<ModelRewriteConfig>('/api/model-rewrite/config'), api<FinalFallbackConfig>('/api/final-fallback/config'), api<ModelRateLimitConfig>('/api/model-rate-limit/config'), api<ClientRateLimitConfig>('/api/client-rate-limit/config'), api<ImageProxyConfig>('/api/image-proxy/config'), api<ImageUsageSummary>('/api/images/usage')]);
      setConfig(c);
      setKeys(k);
      setAudit(a);
      setRewriteConfig(rw);
  ...
  async function savePolicy(k: KeyUsageSummary, form: HTMLFormElement) {
    ...
    setSaving(k.keyId);
    try {
      await api(`/api/keys/${k.keyId}/policy`, { method: 'PATCH', body: JSON.stringify({ tokenLimit: fd.get('tokenLimit') ? Number(fd.get('tokenLimit')) : null, imageDailyLimit: fd.get('imageDailyLimit') ? Number(fd.get('imageDailyLimit')) : null, actionOnLimit: fd.get('actionOnLimit'), resetPolicy: fd.get('resetPolicy'), expiresAt: fromVnInput(fd.get('expiresAt')), usageMultiplier: fd.get('usageMultiplier') ? Number(fd.get('usageMultiplier')) : 1 }) });
      await refresh();
    } finally {
      setSaving('');
    }

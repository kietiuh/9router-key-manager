...
export function AdminKeysSection({ filter, keys, lang, onFilter, onSelect }: { filter: Filter; keys: KeyUsageSummary[]; lang: Lang; onFilter: (filter: Filter) => void; onSelect: (key: KeyUsageSummary) => void }) {
  ...
  return <><section className="toolbar">{ADMIN_FILTERS.map(f => <button key={f} type="button" className={filter === f ? 'active' : ''} onClick={() => onFilter(f)}>{filterLabel(f, lang)}</button>)}</section><section className="tableWrap"><table><thead><tr><th>{t.status}</th><th>{t.name}</th><th>{t.usage}</th><th>{t.tokens}</th><th>{t.daily}</th><th>{t.window}</th><th>{t.action}</th><th>{t.last}</th></tr></thead><tbody>{visible.map(k => <tr key={k.keyId} onClick={() => onSelect(k)}><td><span className={`pill ${k.status}`}>{statusLabel(k.status, lang)}</span></td><td><b>{k.name}</b><br /><code>{k.keyMasked}</code></td><td><div className="meter"><div style={{ width: `${Math.min(k.percentOfLimit ?? 0, 100)}%` }} /></div>{pct(k.percentOfLimit)}</td><td>{fmt(k.total)}</td><td>{fmt(k.tokenLimit)}</td><td>{k.resetPolicy}</td><td>{k.actionOnLimit}</td><td>{vnDateTime(k.lastUsageAt)}</td></tr>)}</tbody></table></section></>;
}

...
export async function buildApp(options: BuildAppOptions = {}) {
  ...
  const sessionMaxAge = options.sessionMaxAge ?? Number(process.env.SESSION_MAX_AGE_SECONDS ?? 60 * 60 * 24 * 30);
  const secureCookie = options.secureCookie ?? (process.env.COOKIE_SECURE === 'true' || (process.env.COOKIE_SECURE !== 'false' && process.env.NODE_ENV === 'production'));
  const allowedOrigins = options.allowedOrigins ?? new Set((process.env.CORS_ORIGINS ?? 'http://localhost:5173,http://127.0.0.1:5173').split(',').map(o => o.trim()).filter(Boolean));
  const nineRouterUpstream = (options.nineRouterUpstream ?? process.env.NINE_ROUTER_UPSTREAM ?? 'http://127.0.0.1:20128').replace(/\/$/, '');
  const app = Fastify({ logger: options.logger ?? true });
  const db = options.db ?? openDb();
  const webRoot = options.webRoot ?? process.env.WEB_ROOT ?? path.resolve(process.cwd(), 'dist/web');
